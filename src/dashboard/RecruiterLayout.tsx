...
import {
  LayoutDashboard, PlusCircle, Sparkles, Search, Star, Send,
  Users, MessageSquare, BarChart3, Bookmark, Building2, Settings,
  Moon, Sun, Code2, X, Menu, Bell, HelpCircle, ShieldCheck, User, LogOut, ChevronDown,
} from 'lucide-react';
import { useTheme } from '@/context/ThemeContext';
import { Avatar } from '@/dashboard/components';
import { recruiterProfile, recruiterCompany } from '@/dashboard/recruiter-data';
import type { RecruiterPageId } from '@/dashboard/recruiter-data';

const navItems: { id: RecruiterPageId; label: string; icon: React.ComponentType<{ className?: string }>; badge?: number }[] = [
  { id: 'rec-home', label: 'Dashboard', icon: LayoutDashboard },
  { id: 'rec-create-role', label: 'Create Role', icon: PlusCircle },
  { id: 'rec-ai-match', label: 'AI Talent Match', icon: Sparkles, badge: 1284 },
  { id: 'rec-discover', label: 'Discover Talent', icon: Search },
  { id: 'rec-shortlisted', label: 'Shortlisted', icon: Star, badge: 86 },
  { id: 'rec-invitations', label: 'Invitations', icon: Send, badge: 4 },
  { id: 'rec-groups', label: 'Candidate Groups', icon: Users },
  { id: 'rec-messages', label: 'Messages', icon: MessageSquare, badge: 2 },
  { id: 'rec-activity', label: 'Hiring Activity', icon: BarChart3 },
  { id: 'rec-saved-searches', label: 'Saved Searches', icon: Bookmark },
...
export function RecruiterSidebar({
  ...
  page: RecruiterPageId;
  setPage: (p: RecruiterPageId) => void;
  mobileOpen: boolean;
  ...
  const handleNav = (p: RecruiterPageId) => {
    setPage(p);
    setMobileOpen(false);
  };
      ...
      <aside className={`fixed lg:sticky top-0 left-0 z-50 lg:z-30 h-screen w-64 flex-shrink-0 bg-card border-r border-default flex flex-col transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="flex items-center justify-between px-5 h-16 border-b border-default flex-shrink-0">
          <div className="flex items-center gap-2.5">
            <div className="relative w-9 h-9 rounded-xl bg-gradient-to-br from-success-500 to-success-700 flex items-center justify-center shadow-glow">
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
          <div className="text-[10px] font-bold text-muted uppercase tracking-wider px-3 mb-2">Recruiter</div>
          {navItems.map((item) => {
            const active = page === item.id;
            return (
              <button key={item.id} onClick={() => handleNav(item.id)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${active ? 'bg-gradient-to-r from-success-500 to-success-600 text-white shadow-soft' : 'text-muted hover:text-body hover:bg-ink-50 dark:hover:bg-ink-800/50'}`}>
                <item.icon className="w-[18px] h-[18px] flex-shrink-0" />
                <span className="flex-1 text-left">{item.label}</span>
                {item.badge && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${active ? 'bg-white/25 text-white' : 'bg-success-500 text-white'}`}>
          ...
          <div className="pt-3 mt-3 border-t border-default space-y-0.5">
            <button onClick={() => handleNav('rec-settings')} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors ${page === 'rec-settings' ? 'bg-gradient-to-r from-success-500 to-success-600 text-white shadow-soft' : 'text-muted hover:text-body hover:bg-ink-50 dark:hover:bg-ink-800/50'}`}>
        ...
        <div className="p-3 border-t border-default flex-shrink-0">
          <div className="flex items-center gap-3 p-2.5 rounded-xl bg-ink-50 dark:bg-ink-800/40">
            <Avatar initials="TN" gradient={recruiterCompany.logoGradient} size="sm" ring={false} />
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-body truncate">{recruiterCompany.name}</div>
              <div className="text-[10px] text-success-600 dark:text-success-400 flex items-center gap-1"><ShieldCheck className="w-3 h-3" /> Verified Company</div>
            </div>
          </div>
        </div>
      </aside>
...
export function RecruiterTopNav({ setMobileOpen, setPage, onLogout }: { setMobileOpen: (o: boolean) => void; setPage: (p: RecruiterPageId) => void; onLogout: () => void }) {
  const { theme, toggle } = useTheme();
    ...
    <header className="sticky top-0 z-30 h-16 bg-card/80 backdrop-blur-xl border-b border-default flex items-center gap-3 px-4 lg:px-6">
      ...
      <div className="flex-1 max-w-md">
        <div className="relative">
          ...
          <input type="text" placeholder="Search candidates, skills, domains..." className="w-full text-sm bg-ink-50 dark:bg-ink-800/40 border border-default rounded-xl pl-10 pr-4 py-2.5 text-body placeholder:text-muted outline-none focus:border-success-400 transition-colors" />
        </div>
      </div>
      <div className="hidden sm:flex items-center gap-2 ml-2">
        <span className="text-sm font-semibold text-body">{recruiterCompany.name}</span>
        <span className="inline-flex items-center gap-1 text-[10px] font-bold text-success-700 dark:text-success-400 bg-success-50 dark:bg-success-950/50 px-2 py-1 rounded-md"><ShieldCheck className="w-3 h-3" /> Verified</span>
      </div>
      <div className="flex items-center gap-1.5 ml-auto">
        ...
        <div className="relative ml-1 flex-shrink-0">
          <button onClick={() => setProfileOpen((open) => !open)} className="inline-flex items-center gap-1.5 rounded-lg p-1 hover:bg-ink-50 dark:hover:bg-ink-800/50" aria-label="Open profile menu" aria-expanded={profileOpen}>
            <Avatar initials={recruiterProfile.initials} gradient={recruiterProfile.gradient} size="sm" ring={false} />
          ...
          {profileOpen && <div className="absolute right-0 top-full mt-2 w-44 rounded-xl border border-default bg-card p-1.5 shadow-float z-50">
            <button onClick={() => { setPage('rec-company'); setProfileOpen(false); }} className="w-full flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm text-body hover:bg-ink-50 dark:hover:bg-ink-800/50"><User className="w-4 h-4 text-muted" /> Profile</button>
